//以下是我之前对coze的agent交互进行的拆解和分析流程。*Q：是‘’我’‘，[用户]的对话内容，*A：是[agent]输出的对话内容，*try 指[agent]在输出对话内容之外进行的对用户可感知的动作，比如搜索网页浏览内容，下载pdf，安排日程，监督其他agent任务状态，分析思维等一些动作。
*Q:
...
class VectorStoreManager:
    def __init__(self, collection_name="insights"):
...
3.6.2 完整PDF生成流程pythonclass CompletePDFReportPipeline:
    ...
    def run_full_pipeline(self, insight: Dict, report_type: str = "pestlied") -> str:
        ...
        pdf_path = self.pdf_generator.generate_pdf(
            markdown_report, 
            chart_files, 
            output_path=f"insight_{report_type}_{int(time.time())}.pdf"
        )
...
四、完整工作流集成pythonclass InsightSystem:
    def __init__(self):
        # 初始化各个组件
